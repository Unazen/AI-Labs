...
dc = pd.read_csv("classification_2params/water_potability.csv")
...
print("after fill", dc.isnull().sum())

# Нормализация числовых данных
scaler = StandardScaler()
dc[numeric_cols] = scaler.fit_transform(dc[numeric_cols])
""" for col in numeric_cols:
    dc[[col]] = scaler.fit_transform(dc[[col]]) """

""" # One-Hot Encoding категорий
df = pd.get_dummies(dc, columns=categorical_cols, drop_first=True) """

# Проверка результата
print("\nПервые строки обработанных данных:")
print(dc.head())

dc.to_csv("classification_2params/processed_classification_2params.csv", index=False)
